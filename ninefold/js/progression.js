...
export function replanSets({ set, rx, implement, location, equip, exerciseId, allowUp = true }) {
  // The weekly engine steps a lower-body lift by 5 kg and everything else by
  // 2.5 (nextLoadUp/loadDown's `lower`). Without the id this always used 2.5,
  // so a squat that missed the range mid-session backed off half as far as the
  // same miss would have moved it next week.
  const lower = LOWER.has(exerciseId || (rx && rx.exerciseId));
  ...
  if (shortfall >= 2 || (shortfall >= 1 && set.rir === 0)) {
    const load = loadDown(set.weightKg, implement, location, equip, lower);
  ...
  if (allowUp && set.reps >= range.hi + 2 && (set.rir == null || set.rir >= 2)) {
    const load = nextLoadUp(set.weightKg, implement, location, equip, lower);
