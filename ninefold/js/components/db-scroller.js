...
import { rackAt } from "../progression.js";
...
export function WeightStepper(implement, equip, location, initial, onChange, profile) {
  ...
  if (implement === "cable") {
    mode = "cont";
    // ⚠ THE PLACE YOU ARE IN, not the first place you described. `equip.cable`
    // is the flat fallback (places[0]'s stack); the engine rounds through
    // rackAt(equip, location), so reading the flat one here offered increments
    // that do not exist at a hotel or any second gym — and the number you then
    // logged was one the stack could not make.
    const cable = rackAt(equip, location).cable || {};
    cfg = { min: cable.minKg ?? 2.5, max: cable.maxKg ?? 120,
      step: cable.stepKg ?? 2.5, micro: cable.microStepKg };
  } else {
    // `equip.dumbbells` is a MAP of places. Falling back to it when this place
    // has no entry read minKg/maxKg/stepKg off that map — all undefined — and
    // silently invented a 30 kg cap in 0.5 kg steps. An unknown rack has no cap.
    const db = (equip.dumbbells && equip.dumbbells[location]) || {};
    if (Array.isArray(db.valuesKg) && db.valuesKg.length) {
      mode = "disc";
      values = [...db.valuesKg].sort((a, b) => a - b);
    } else {
      mode = "cont";
      cfg = { min: db.minKg != null ? db.minKg : 0, max: db.maxKg ?? Infinity, step: db.stepKg ?? 2.5 };
    }
  }
