...
export async function mergeRestore(data, { overwrite = false } = {}) {
  ...
  // ⚠ A WIPED DEVICE HAS TO COME BACK AS THE SAME INSTALL. The boot restore is
  // the path that rebuilds a wiped phone (app.js), and it merges without
  // `overwrite`, so it used to leave the id alone — then getInstallId() minted a
  // fresh one on the first push and the Worker refused every push after that as
  // a foreign install. Silent: the app looks healthy and nothing is backed up.
  // An install that already HAS an id keeps it; only an empty device adopts.
  if (overwrite) await adoptInstallId(data.installId);
  else if (data.installId && !(await db.getPref("installId"))) await adoptInstallId(data.installId);
  return added;
}
