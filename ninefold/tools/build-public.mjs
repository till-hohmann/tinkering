...
//
// `audio/yoga` is the same category as those masters: ~150 MB of RENDERED
// narration across three experience levels, regenerated by tools/build-voice.py
// from js/yoga/cues.js. The cues are the reviewable, diffable source and they
// ship; the audio is a build artefact and has no business in a git history that
// strangers clone. It is gitignored in the dev repo for the same reason, and
// this tool copies from DISK, so it needs the rule here too.
const EXCLUDE_RE = /(^|[\\/])(\.wrangler|node_modules|__pycache__|\.deploy|overlay|concepts)([\\/]|$)|\.pyc$|(^|[\\/])img[\\/]exercises[\\/][^\\/]+\.png$|(^|[\\/])audio[\\/]yoga([\\/]|$)/;
