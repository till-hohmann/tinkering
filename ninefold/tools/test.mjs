...
// ---------------------------------------------------------------------------
// v196 — the sibling-path audit: one rule, every path that needs it
// ---------------------------------------------------------------------------
group("dates — one calendar day, one implementation", () => {
  // Every .js under js/, with its repo-relative path.
  const jsFiles = () => {
    const walk = (dir) => readdirSync(new URL(`../${dir}/`, import.meta.url), { withFileTypes: true })
      .flatMap((e) => (e.isDirectory() ? walk(`${dir}/${e.name}`)
        : e.name.endsWith(".js") ? [{ rel: `${dir}/${e.name}`, path: new URL(`../${dir}/${e.name}`, import.meta.url) }] : []));
    return walk("js");
  };
  it("adds days locally, across months, years and a DST change", () => {
    ...
    // European clocks go back on 25 October 2026 and forward on 29 March.
    assert.equal(addDaysISO("2026-10-24", 3), "2026-10-27");
    assert.equal(addDaysISO("2026-03-28", 2), "2026-03-30");
    assert.equal(addDaysISO(null, 3), null);
  });
  it("yesterday is one day back, not two", () => {
    assert.equal(prevISO("2026-09-24"), "2026-09-23");
    assert.equal(prevISO("2026-01-01"), "2025-12-31");
  });
  it("counts whole days, including across a clock change", () => {
    assert.equal(daysBetweenISO("2026-09-14", "2026-09-21"), 7);
    assert.equal(daysBetweenISO("2026-09-21", "2026-09-14"), -7);
    assert.equal(daysBetweenISO("2026-10-24", "2026-10-27"), 3);
  });
  it("todayISO reads the LOCAL day of a Date", () => {
    // 00:30 local on the 24th is the 23rd in UTC for any negative offset and the
    // 24th here either way — the point is that it reads the local fields.
    assert.equal(todayISOof(new Date(2026, 8, 24, 0, 30)), "2026-09-24");
    assert.equal(todayISOof(new Date(2026, 8, 24, 23, 45)), "2026-09-24");
  });
  it("NO MODULE GROWS ITS OWN. A date is never taken through toISOString", () => {
    // This is the guard, not the tests above: the bug was a fourth copy of
    // addDaysISO that went through UTC and moved every imported week start back
    // a day. `new Date().toISOString()` for a TIMESTAMP is fine; slicing a
    // calendar day out of one is not.
    const offenders = [];
    for (const f of jsFiles()) {
      const src = readFileSync(f.path, "utf8");
      if (f.rel === "js/dates.js") continue;
      if (/toISOString\(\)\s*\.\s*(slice|substring)\(\s*0\s*,\s*10\s*\)/.test(src)) offenders.push(`${f.rel}: date sliced out of a UTC timestamp`);
      if (/^\s*(export\s+)?(function|const)\s+(addDays|addDaysISO|addDaysLocal|prevISO|daysBetween|daysBetweenISO)\b/m.test(src))
        offenders.push(`${f.rel}: defines its own date helper — import it from dates.js`);
    }
    assert.deepEqual(offenders, [], offenders.join("; "));
  });
});

group("mobility — a hold you take to failure is not a stretch", () => {
  const item = (id, name, durationSeconds) => ({ id, name, mode: "timed", durationSeconds, bilateral: true });
  const rec = (id, sec) => [{ id, side: "Left", heldSec: sec }, { id, side: "Right", heldSec: sec }];
  const run = (it, seconds) => {
    let state = {};
    for (const s of seconds) state = mobilityHoldResults(state, [it], rec(it.id, s)).state;
    return state[it.id].targetSec;
  };

  it("a side plank does not lose its target to one short session", () => {
    const sp = item("side_plank", "Side plank", 35);
    assert.equal(run(sp, [20]), 35);
    assert.equal(run(sp, [20, 22]), 35);
  });
  it("three bad sessions re-base it to the BEST of the three, not the last", () => {
    const sp = item("side_plank", "Side plank", 35);
    assert.equal(run(sp, [20, 24, 21]), 25);       // best 24 → 25 after rounding
  });
  it("a good session in between clears the count", () => {
    const sp = item("side_plank", "Side plank", 35);
    assert.equal(run(sp, [20, 35, 20, 20]), 35);
  });
  it("the wall sit, Copenhagen, bird dog and side-lying leg raise are the same kind", () => {
    for (const [id, name] of [["wall_sit", "Wall sit"], ["copenhagen", "Copenhagen plank"],
                              ["bird_dog", "Bird dog"], ["sl_hip_abduction", "Side-lying leg raise"]]) {
      assert.equal(run(item(id, name, 60), [30]), 60, `${id} dropped its target on one short session`);
    }
  });
  it("a genuine stretch still re-bases on the day, which is right for a stretch", () => {
    assert.equal(run(item("couch_stretch", "Hip flexor (couch)", 40), [20]), 20);
    assert.equal(run(item("hip_9090", "90/90 hip switch", 75), [40]), 40);
  });
  it("and both kinds still earn a raise the same way", () => {
    assert.equal(run(item("side_plank", "Side plank", 35), [35, 35]), 40);
    assert.equal(run(item("couch_stretch", "Couch", 40), [40, 40]), 45);
  });
});

group("engines — the rules that were only on one path", () => {
  const equip = { barWeightKg: 20, barbellPlatesKg: [25, 20, 15, 10, 5, 2.5, 1.25],
    dumbbells: { Home: { valuesKg: [20, 22, 24, 26, 28, 30, 32] }, Gym: { minKg: 5, maxKg: 22.5, stepKg: 2.5 } } };

  it("mid-session replanning steps a lower-body lift by the lower-body step", () => {
    const miss = { reps: 4, weightKg: 100, rir: 0 };
    const rx = { repRange: "6-8" };
    const squat = replanSets({ set: miss, rx, implement: "barbell", location: "Home", equip, exerciseId: "back_squat" });
    const bench = replanSets({ set: miss, rx, implement: "barbell", location: "Home", equip, exerciseId: "bench_press" });
    assert.equal(squat.load, 95);
    assert.equal(bench.load, 97.5);
  });
  it("it also reads the id off the prescription when the caller passes neither", () => {
    const up = replanSets({ set: { reps: 12, weightKg: 100, rir: 3 }, rx: { repRange: "6-8", exerciseId: "back_squat" },
      implement: "barbell", location: "Home", equip });
    assert.equal(up.load, 105);
  });
  it("a top set is a work set: a timed hold never counts as one", () => {
    assert.equal(topSetWeight({ sets: [{ weightKg: 40, reps: null, seconds: 60 }, { weightKg: 22.5, reps: 10 }] }), 22.5);
    assert.equal(topSetWeight({ sets: [{ weightKg: 40, reps: null, seconds: 60 }] }), 40, "a timed-only entry still reports its load");
    assert.equal(topSetWeight(null), 0);
  });
  it("a load is written the way it was lifted, wherever it is printed", () => {
    assert.equal(fmtLoad("dumbbell_pair", 27), "2×27 kg");
    assert.equal(fmtLoad("barbell", 100), "100 kg");
    assert.equal(fmtLoad("bodyweight", 0), "BW");
  });
});

group("backup and plan import — what the sibling path used to drop", () => {
  it("an exported backup carries the install identity the restore adopts", () => {
    const withId = JSON.parse(buildBackup([], [], "2026-09-24", null, "inst-abc"));
    assert.equal(withId.installId, "inst-abc");
    // Without one (an older export) the field is simply absent, not null.
    assert.ok(!("installId" in JSON.parse(buildBackup([], [], "2026-09-24"))));
  });
  it("updating a block from a spreadsheet keeps the week prose the CSV cannot carry", () => {
    const base = { id: "b", name: "B", startDate: "2026-09-14",
      exercises: { back_squat: { name: "Squat", cue: "", implement: "barbell" } },
      dayTemplates: { Tue: { weekday: "Tue", type: "strength", location: "Home", label: "Lower",
        exercises: [{ exerciseId: "back_squat", role: "compound", restSeconds: 150 }] } },
      weeks: [{ weekNumber: 1, startDate: "2026-09-14", phaseName: "Base", scheme: "4x6", intensity: "RPE 8",
        focus: "Controlled sixes, not grinders.",
        days: { Tue: { weekday: "Tue", type: "strength", exercises: [
          { exerciseId: "back_squat", role: "compound", prescribedSets: 4, repRange: "6", restSeconds: 150 }] } } }] };
    const revised = JSON.parse(JSON.stringify(base));
    revised.weeks[0].days.Tue.exercises[0].prescribedSets = 5;
    const next = applyPlanCSV(base, fromCSV(toCSV(revised)), { mode: "update" });
    assert.equal(next.weeks[0].days.Tue.exercises[0].prescribedSets, 5, "the change itself must land");
    assert.equal(next.weeks[0].focus, "Controlled sixes, not grinders.");
    assert.equal(next.weeks[0].scheme, "4x6");
    assert.equal(next.weeks[0].intensity, "RPE 8");
  });
  it("week start dates stay on the day they belong to", () => {
    const base = { id: "b", name: "B", startDate: "2026-09-14", exercises: {}, dayTemplates: {},
      weeks: [1, 2, 3].map((n) => ({ weekNumber: n, startDate: "", phaseName: "W" + n,
        days: { Tue: { weekday: "Tue", type: "strength", exercises: [] } } })) };
