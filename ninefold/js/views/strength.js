...
function pickExercise(program, { excludeId = null, matchFor = null, equip, location,
  ...
  return new Promise((res) => {
    ...
    const hidden = new Set([...(exclude || []), ...(excludeId ? [excludeId] : [])]);
    ...
    function draw(q) {
      ...
      const shownAlts = alts.filter((id) => !hidden.has(id) && hit(id));
      if (shownAlts.length) {
      ...
      const rest = [...everything]
        .filter((id) => !hidden.has(id) && !altSet.has(id) && hit(id) && (showAll || usable.has(id)))
        .sort(byName);
...
export async function runStrength(container, program, day, weekday, iso, location, opts = {}) {
  ...
  try {
    for (const s of await getAllSessions()) {
      // Before TODAY, not before the plan day. `iso` is srcIso: replaying a
      // week-2 day in week 5 cut four weeks of real sessions out of the
      // baseline, so ordinary working sets fired the personal-best celebration.
      if (!s.strengthResult || (s.date && s.date >= (opts.logDate || iso))) continue;
  ...
  async function makeEntry(exId, baseRx, seed = null) {
    ...
    // READINESS EASING APPLIES TO A LIFT YOU PICK MID-SESSION TOO. It was applied
    // once, to the list built at the top, so swapping bench for a DB press on a
    // Drained day handed back the full un-eased target while every other lift in
    // the session was 10% down.
    if (rd && rd.mult && rd.mult !== 1 && rec.load != null) {
      rec.load = roundLoad(rec.load * rd.mult, lib.implement, location, equip);
      rec.eased = true;
    }
    if (rd && rd.dropSet) rx.prescribedSets = Math.max(2, (rx.prescribedSets || 3) - 1);
  ...
  async function addExercise() {
    // Excluding what the day already holds: a second slot for the same lift
    // makes every id-keyed record (set deltas, deviation questions, the restore
    // of already-logged sets) ambiguous against the slot-keyed results.
  ...
  function initSets(rx, prevEx, implement, rec) {
    ...
    // ⚠ ONLY THE ENGINE'S NUMBER WAS ROUNDED TO THIS RACK. The fallbacks — last
    // session's top set (possibly taken at a heavier rack) and the block's load
    // anchor — went in raw, so the first occurrence of a lift in a new block at
    // a lighter place prefilled a weight the room cannot load, and the warm-up
    // ramped off it.
    const fallbackW = prevW != null ? prevW : aw != null ? aw : baseW;
    const fillW = recW != null ? recW
      : (implement === "bodyweight" ? fallbackW
        : Math.min(roundLoad(fallbackW, implement, location, equip) ?? fallbackW,
                   loadCeiling(implement, location, equip) ?? Infinity));
    ...
    // This slot's own record first: `results` is keyed by slot since v193, and a
    // day can legitimately hold the same lift twice in an imported plan.
    const prior = results.find((r) => r._i === exIndex) || results.find((r) => r.exerciseId === rx.exerciseId);
    if (prior) {
      for (let i = 0; i < prior.sets.length && i < sets.length; i++) {
        ...
        sets[i] = { ...sets[i], weightKg: ps.weightKg,
          ...
          // Both sides of a two-sided hold, or the next commit drops what the
          // clock measured (every superset hand-over re-enters and re-commits).
          ...(Array.isArray(ps.sides) ? { sides: ps.sides } : {}),
  ...
  function renderExercise() {
    ...
    function logSet() {
      ...
      if (!state.timed && state.sets.some((x) => !x.done)) {
        const plan = replanSets({ set: s, rx, implement, location, equip, exerciseId: rx.exerciseId,
