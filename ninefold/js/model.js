...
export function topSetWeight(loggedExercise) {
  ...
  // Timed holds carry a weight but no reps, and they are not a "top set" of
  // anything — progression.js topSet and records.js both skip them, and this
  // sibling did not, so a weighted plank could out-rank a working set.
  const work = loggedExercise.sets.filter((s) => s.reps != null);
  const sets = work.length ? work : loggedExercise.sets;
...
/**
 * A LOAD, written the way it was lifted: "2×27kg" for a dumbbell pair, "BW" for
 * bodyweight. `setDisplay` needs a whole set (it prints reps); the top-set
 * readouts on Progress and the week summary have only a number, and printing it
 * bare read a pair as half the lift.
 */
export function fmtLoad(implement, kg) {
  const w = Number(kg) || 0;
  if (implement === "bodyweight" || (w === 0 && implement !== "cable")) return "BW";
  const shown = fmtWeight(w);
  return implement === "dumbbell_pair" ? `2×${shown}` : shown;
}
