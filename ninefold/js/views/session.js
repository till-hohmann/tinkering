...
async function runSession({ program, weekNumber, weekday, week, day, template, srcIso, logDate }) {
  ...
  try {
    ...
    const hasProgress = saved && ((saved.strengthResult && saved.strengthResult.length) || saved.cardioResult
      || (saved._exIndex | 0) > 0 || (saved._sub && (saved._sub.results || []).length > 0));
  ...
  const strengthPhase = (loc, rd) =>
    new Promise((res, rej) => runStrength(stage, program, dayWithPairs, weekday, srcIso, loc || template.location, {
      ...
      readiness: rd, adhocPlace, logDate,
    ...
    if (day.type === "strength") {
      if (substituting) {
        const { strengthResult, substitution } =
          await substitutedStrength(stage, program, dayWithPairs, weekday, srcIso, plannedLoc, actualLoc, readiness, adhocPlace, logDate, {
            // Progress is kept as the SUBSTITUTE rows plus the swap choices, which
            // is what resuming needs; the planned-lift conversion happens when the
            // session ends or is left.
            onProgress: (sub) => {
              draft._sub = { results: sub.results, index: sub.index, choices: sub.choices };
              draft.strengthResult = sub.converted.strengthResult;
              draft.substitution = { plannedLocation: plannedLoc, actualLocation: actualLoc, items: sub.converted.items };
              persist();
            },
            onExit: (kind, converted) => {
              if (converted) { draft.strengthResult = converted.strengthResult; draft.substitution =
                { plannedLocation: plannedLoc, actualLocation: actualLoc, items: converted.items }; }
              persist();
            },
            resume: resuming ? draft._sub : null,
          });
...
async function substitutedStrength(stage, program, day, weekday, iso, plannedLoc, actualLoc, readiness, adhocPlace, logDate,
                                   { onProgress, onExit, resume } = {}) {
  ...
  // 3) preview + let the user swap any substitute. On a resume the choices were
  //    already made — re-applying them and going straight back to the workout is
  //    the point of resuming.
  if (resume && resume.choices) {
    for (const p of plan) if (resume.choices[p.originalId]) p.subId = resume.choices[p.originalId];
  } else {
  ...
  // ⚠ THIS PATH USED TO KEEP EVERYTHING IN MEMORY UNTIL THE LAST SET. The planned
  // path persists after each exercise and can be left with the X button; the
  // substituted one passed neither hook, so a phone call, a reload or iOS
  // evicting the tab in the middle of a session away from home lost every set —
  // and the resume prompt never appeared, because nothing had been written.
  const choices = Object.fromEntries(plan.map((p) => [p.originalId, runId(p)]));
  const subResults = await new Promise((res, rej) =>
    runStrength(stage, augProgram, { exercises: subExercises, supersets }, weekday, iso, actualLoc,
      { exercises: subExercises, recs: subExercises.map((e) => e._rec), onComplete: res, readiness, adhocPlace,
        logDate,
        seed: resume && Array.isArray(resume.results) ? resume.results : null,
        startIndex: resume && resume.index ? resume.index : 0,
        // Both shapes: the substitute rows (what a resume needs to carry on) and
        // the converted planned-lift rows (what "complete what's done" saves if
        // the app never comes back to this screen).
        onProgress: (results, nextIndex) => onProgress
          && onProgress({ results, index: nextIndex, choices, converted: assemble(results) }),
        onExit: onExit ? (kind, results) => {
          // Hand back what is logged, already converted onto the planned lifts,
          // so leaving early saves a real session rather than substitute rows.
          onExit(kind, kind === "discard" || !Array.isArray(results) ? null : assemble(results));
          rej(new SessionExit(kind));
        } : undefined,
      }));

  const { strengthResult, items } = assemble(subResults);
  ...
  //    Declared as a function so the exit path above can call it too: leaving
  //    early must write the same rows finishing would.
  function assemble(subResults) {
  const byIdx = {};
