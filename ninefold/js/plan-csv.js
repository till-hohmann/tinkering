...
export function applyPlanCSV(base, parsed, { mode = "update", id, startDate } = {}) {
  ...
  return {
    ...
    exercises,
    dayTemplates,
    // The CSV carries sets, reps, rest and roles. It does NOT carry the week's
    // scheme, intensity or focus text — the Today card and the day view read
    // those — so they are taken from the block being updated rather than
    // dropped, which is what "keeps everything the CSV cannot express" promised.
    weeks: parsed.weeks.map((w, i) => {
      const before = mode === "update"
        ? (src.weeks || []).find((x) => x.weekNumber === w.weekNumber) : null;
      return {
        ...(before ? { scheme: before.scheme, intensity: before.intensity, focus: before.focus } : {}),
        ...w,
