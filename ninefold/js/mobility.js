...
export function applyHoldResults(state, items, holds, { eased = false } = {}) {
  // (isStrengthHold below: the failure-limited holds in this routine are
  //  copenhagen, wall sit, side plank, bird dog and the side-lying leg raise.)
  const next = { ...(state || {}) };
  const changes = [];
  ...
  for (const it of items) {
    ...
    if (actual >= target) {
      ...
      // ⚠ A STRENGTH HOLD ENDS WHEN IT ENDS. The same rule that was wrong for a
      // dead hang in v186 was still live here: a side plank, wall sit,
      // Copenhagen, bird dog or side-lying leg raise is held to failure, so
      // falling short IS the exercise, and lowering the target on one short
      // session walks it down for ever. Those take the holds.js rule — three
      // bad sessions in a row, then re-based to the BEST of the three — while a
      // genuine stretch still re-bases on the day, which is right for a stretch.
      st.streak = 0;
      if (isStrengthHold(it)) {
        st.misses = [...(st.misses || []), actual];
        if (st.misses.length >= 3) {
          const rebased = round5(Math.max(...st.misses));
          st.misses = [];
          if (rebased < target) { st.targetSec = rebased; changes.push(`${it.name} ↓ ${rebased}s`); }
        }
      } else {
        st.targetSec = round5(actual);
