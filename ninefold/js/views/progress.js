...
import { addDaysISO as addDaysLocal } from "../dates.js";
import { getAllPrograms, getActiveProgram, getAllSessions, getVO2maxLog, getNutritionLog, getBodyweight, getMeasurementsLog, getDexaLog, getDexaBooked, getWeightLog, equipmentForProgram } from "../store.js";
...
// Any block's name for the lift, then the library's: this screen spans every
// block, so a lift the CURRENT one dropped used to render as `one_arm_db_row`.
let allProgramsForNames = [];
const exName = (program, id) =>
  ((program && program.exercises && program.exercises[id]) || {}).name
  || (allProgramsForNames.find((p) => p.exercises && p.exercises[id]) || { exercises: {} }).exercises[id]?.name
  || (metaFor(program || { exercises: {} }, id) || {}).name
  || id;
...
export async function renderProgress() {
  ...
  if (program && strength.length) {
    ...
    // BY DATE, like the Today card counts it — not by the week number stored on
    // the session. "Do this workout now" replays another week's day and logs it
    // under today with THAT week's number, so a real session you just finished
    // was missing from these bars while Today's dots showed it.
    const wkStart = M.weekStartFor(program, M.todayISO());
    const wkEnd = addDaysLocal(wkStart, 7);
    const loggedSets = setsFromResults(strength
      .filter((s) => s.programId === program.id && s.date >= wkStart && s.date < wkEnd)
      .flatMap((s) => s.strengthResult || []));
    ...
    if (rows.length) {
      ...
      children.push(el("div.card.flush", {}, [
        ...
        // A deload or taper is SUPPOSED to read light. Saying so stops the
        // reader adding back the sets the week exists to remove — the builder's
        // own audit excludes these weeks for the same reason.
        M.isReducedPhase(((program.weeks || []).find((w) => w.weekNumber === curWeek) || {}).phaseName)
          ? el("p.note", { style: "margin:6px 2px 2px;color:var(--cyan)", text: "This is a reduced week — light chips are the plan working, not a gap." })
          : null,
      ]));
  ...
  if (inRange.length) {
    ...
    // Distinct calendar weeks trained, not distinct week NUMBERS — the latter
    // repeat every block and under-count a long history.
    const weeksActive = new Set(inRange.map((s) => s.date.slice(0, 8) + String(Math.ceil(Number(s.date.slice(8, 10)) / 7)))).size;
    ...
    let trendMini;
    if (strength.length && (program ? strength.some((s) => s.programId === program.id) : true)) {
      // ONE BLOCK'S WEEKS. `strength` deliberately spans every block, so keying
      // by week number alone added block 1's W3 to block 2's W3 and called the
      // sum "W3" — and "Weeks" counted 8 for sixteen weeks of training.
      const trendSessions = program ? strength.filter((s) => s.programId === program.id) : strength;
      const byWeek = new Map();
      for (const s of trendSessions) byWeek.set(s.weekNumber, (byWeek.get(s.weekNumber) || 0) + M.sessionVolume(s));
  ...
  try { allProgramsForNames = await getAllPrograms(); } catch (_) { allProgramsForNames = []; }
  const lifts = [...byKey.entries()]
    ...
    .map((l) => {
      ...
      // metaFor, and the place actually trained at: reading the implement off
      // the program alone yields undefined for a substitute or library lift, and
      // either miss returns a null ceiling — which is how a lift pinned at the
      // rack's heaviest dumbbell showed a "stall" it cannot escape.
      const ceiling = loadCeiling(metaFor(program, l.exId).implement, lastOcc.loc, progEquip);
      ...
      el("div.card.tight", {}, [
        el("div.row", { style: "gap:12px" }, [
          ...
          el("div", { style: "flex:1;min-width:0" }, [
            ...
            el("div.note", { // setDisplay, not fmtWeight: a dumbbell pair is written "2×27kg" everywhere
              // else, and printing the per-hand number bare reads as half the lift.
              text: `${M.fmtWeight(Math.round(l.last))} est. 1RM · ${M.fmtLoad(metaFor(program, l.exId).implement, l.lastTop)} top · ${l.occ.length}×${l.multiDay ? " · " + l.weekday : ""}` }),
          ]),
          l.stall
...
async function yogaGapNote(program, curWeek, loggedSets) {
  ...
  // Only muscles this program actually trains: the bars above hide untrained
  // ones, so listing them here blamed yoga for a gap the plan never had.
  const planned = plannedSetsByMuscle(program, curWeek);
  const short = MUSCLES.filter((m) => LANDMARKS[m] && LANDMARKS[m].mev > 0
    && (planned[m] || 0) > 0 && (loggedSets[m] || 0) < LANDMARKS[m].mev);
