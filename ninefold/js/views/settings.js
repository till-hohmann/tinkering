...
import { getActiveProgram, getAllPrograms, getAllSessions, importProgram, restoreBackup, getProgram,
  ...
  getZoneBounds, setZoneBounds, syncedPrefs, getInstallId, getVO2maxLog, addVO2max,
...
export async function renderSettings() {
  ...
  async function doExport(kind) {
    const iso = todayISO();
    const isMd = kind === "md";
    const text = isMd ? buildMarkdownLog(programs, sessions, iso) : buildBackup(programs, sessions, iso, await syncedPrefs(), await getInstallId());
    ...
    setChildren(dangerBody,
      ...
      cfgHasBackup
        ? el("p.note", { style: "margin-top:8px", text:
            "Your cloud backup is NOT touched — it's on your own Worker, so clear it there if you want it gone. This device forgets the endpoint and token with everything else, so re-entering them in Settings is what brings the data back." })
        : null,
