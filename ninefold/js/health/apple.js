...
const provider = {
  ...
  async status() {
    ...
    // WHICH METRICS ACTUALLY ARRIVED. The Shortcut is hand-built, the Worker
    // accepts any JSON, and a day object only needs a `date` to be stored — so a
    // Shortcut using plausible-but-wrong key names ("restingHeartRate" instead of
    // "restingHR") pushes successfully, reports a fresh timestamp, and delivers
    // nothing. Without this the bridge would look healthy while every card stayed
    // empty, which is the most confident way to be broken.
    const KNOWN = ["restingHR", "hrv", "sleepHours", "weightKg", "activeKcal", "basalKcal", "vo2max"];
    const recent = dates.slice(-7).map((d) => daysOf(store)[d]);
    const seen = KNOWN.filter((k) => recent.some((d) => d && Number.isFinite(Number(d[k]))));
    const unknownKeys = [...new Set(recent.flatMap((d) => Object.keys(d || {})))]
      .filter((k) => k !== "date" && k !== "workouts" && !KNOWN.includes(k) && k !== "heightM" && k !== "maxHR");
    ...
    return {
      ...
      metrics: seen,
      // Arriving under names the app doesn't read. Named individually, because
      // "sleepHrs" vs "sleepHours" is invisible until someone puts them side by side.
      unrecognised: unknownKeys.slice(0, 6),
      mismatched: seen.length === 0 && unknownKeys.length > 0,
    };
  },
