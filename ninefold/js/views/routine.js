...
export function runRoutine(container, def, program, opts = {}) {
  ...
  function voiceCue(step) {
    ...
    // Same test as the live cue above (kindOf): a strength hold is a hold. This
    // half of the pair only asked about stretches, so a plank, wall sit or side
    // plank spoke with the screen on and went silent with it locked.
    if (step.type === "timed" && (isStretch(step.item) || isStrengthHold(step.item))) return "stretch";
    return null;
  }
